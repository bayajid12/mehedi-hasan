...
export default function MediaAndTalks() {
  ...
  return (
    <section className="w-full bg-[#E5E5E4] py-24 px-6 md:px-12 lg:px-20 flex flex-col items-center border-t border-gray-300/50">
      {/* Section Header */}
      <div className="flex flex-col items-center mb-16 text-center">
        <span className="font-bebas text-xl md:text-2xl text-gray-500 tracking-widest mb-2">
          MEDIA & TALKS
        </span>
        <h2 className="font-bebas text-5xl md:text-7xl lg:text-[5.5rem] leading-[0.85] text-[#2b2b2f] tracking-wide uppercase">
      ...
      <div className="max-w-[1400px] w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {posts.map((post, index) => (
          <div 
            key={index} 
            className="flex flex-col border border-gray-300/60 rounded-2xl overflow-hidden hover:shadow-xl transition-shadow duration-300 cursor-pointer bg-white/40 backdrop-blur-sm"
          >
            {/* Card Image */}
            ...
            <div className="p-5 md:p-6 flex flex-col flex-grow">
              <span className="font-inter text-gray-500 text-[10px] md:text-xs font-semibold tracking-wider uppercase mb-2">
                {post.category}
              </span>
              <h3 className="font-bebas text-xl md:text-2xl leading-tight text-[#111111] uppercase">
                {post.title}
              </h3>
            </div>
          </div>
